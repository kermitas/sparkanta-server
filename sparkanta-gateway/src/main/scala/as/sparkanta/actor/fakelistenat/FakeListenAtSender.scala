...
class FakeListenAtSender(amaConfig: AmaConfig) extends Actor with ActorLogging {
  ...
  protected val listenAt = new ServerSocket.ListenAt(new IdentifiedInetSocketAddress(0, "192.168.2.25", 8080), 5 * 1000, 2 * 60 * 1000)

  override def preStart(): Unit = try {
    context.system.scheduler.schedule(1 seconds, 2 * 60 - 5 seconds, self, true)
