...
class FakeListenAtSender(amaConfig: AmaConfig) extends Actor with ActorLogging {
  ...
  protected val listenAt = new ServerSocket.ListenAt(new IdentifiedInetSocketAddress(0, "localhost", 8080), 5 * 1000, 30 * 1000)

  override def preStart(): Unit = try {
    context.system.scheduler.schedule(2 seconds, 25 seconds, self, true)
